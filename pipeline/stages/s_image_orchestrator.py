...
def generate_with_qa(
    ...
    Returns:
        Path to final saved image (PNG), or None if generation failed entirely.
    """
    logger.info("[image-orchestrator] Starting for slug=%s, character=%s", slug, character)
    ...
    if best_image and best_image.exists():
        ...
        canonical = best_image.parent / f"{slug}.png"
...
def _finalize(img_path: Path, slug: str) -> None:
    """Copy the chosen image to the canonical slug.png path."""
    canonical = img_path.parent / f"{slug}.png"
