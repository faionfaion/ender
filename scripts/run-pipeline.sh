#!/bin/bash
# Ender pipeline runner — called by cron.
#
# Cron schedule:
#   0 7 * * *        bash ~/workspace/projects/ender-faion-net/scripts/run-pipeline.sh generate
#   5 9,11,14,17 * * * bash ~/workspace/projects/ender-faion-net/scripts/run-pipeline.sh publish
#   5 19 * * *       bash ~/workspace/projects/ender-faion-net/scripts/run-pipeline.sh digest

set -euo pipefail

MODE="${1:-generate}"
PROJECT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
LOCK="/tmp/ender-${MODE}.lock"
